...
img.onload = function () {
    ...
    let playerlife = getData("playerLife1_red")
    //玩家控制飞船类
    class spaceship {
        constructor(name) {
    ...
    function draw() {
        ...
        for (var i in rocks) {
            ...
            var sc = {
                ...
                y: ship.y + ship.canvas.height / 2
            }
            if (r.hp < 0) {
                grade += r.canvas.width //击毁得分
                shootspeed-- //连续击毁20个陨石射速提高10%
                rocks.splice(i, 1)
                for (var m = 0; m < r.canvas.width; m++) {
                    fires.push(new fire(rc.x, rc.y))
                }
            }
            if (Math.sqrt(Math.pow(rc.x - sc.x, 2) + Math.pow(rc.y - sc.y, 2)) < r.canvas.width / 2 + ship.canvas.width / 2 - 50) {
                run = false
                ships--
                shootspeed = 200 //恢复默认射速
                for (var m = 0; m < 200; m++) {
        ...
        if (rocks.length < 10 + grade / 1000) { //每得到1000分增加一块陨石
        ...
        for (var i = 0; i < ships; i++) {
            ctx.drawImage(img,
                playerlife[0], playerlife[1], playerlife[2], playerlife[3],
                playerlife[2] * i * 1.2 + 20, 20, playerlife[2], playerlife[2])
        }
        if (!run) {
        ...
        ctx.textAlign = "left"
        ctx.fillStyle = "white"
        ctx.fillText(grade, 20, 80)
        ctx.fillText(shootspeed, 20, 120)
